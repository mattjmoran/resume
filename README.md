# Resume

A LaTeX-based resume template with resources and best practices for creating an
effective resume.

## Quick Links

- [View Documentation & Resources][GitHub Pages]
- [Download Latest Resume (PDF)][Resume PDF]

## Attributions

See the [attributions page][Project Attributions] for open-source projects that
contributed to this project.

## License

This project is distributed under the following licenses:

- The software, code, templates, and technical implementation contained within
  this repository are licensed under the terms of the [MIT License](LICENSE-MIT)
  (SPDX License Identifier: `MIT`).
- The written prose, documentation, guides, and checklists contained within this
  repository are licensed under the terms of the Creative Commons
  [CC-BY-NC-SA v4.0 license](LICENSE-CC-BY-NC-SA)
  (SPDX License Identifier: `CC-BY-NC-SA-4.0`).

### Contribution

Unless you explicitly state otherwise, any contribution intentionally submitted
for inclusion in the work by you shall be licensed as above, without any
additional terms or conditions.

[GitHub Pages]: https://mattjmoran.github.io/resume/
[Resume PDF]: https://github.com/mattjmoran/resume/releases/download/latest/resume.pdf
[Project Attributions]: https://mattjmoran.github.io/resume/attributions
